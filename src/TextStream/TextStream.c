...
printf ("\n____________________________________\n");
    ...
    outer = createList(toplevel, "");
    inner1 = createObject(outer, "");
    addString(inner1, "feature", "Fabulous New Feature");
    addString(inner1, "version", "2001.020");
    addString(inner1, "releasedate", "01-jun-2029");
    addNumber(inner1, "available", 121);
    addString(inner1, "preferences", NULL);
    users = createList(inner1, "users");
    user = createObject(users, "user");
    addString(user, "user", "Ben Painter");
    addNumber(user, "number", 10);
    addString(user, "versionrequested", "2011.020");
    destroy(user);
    addString(inner1, "onemore", NULL);
    destroy(users);
    destroy(inner1);
    inner1 = createObject(outer, "");
    addString(inner1, "foo", "bar");
    addString(inner1, "version", "2001.020");
    addString(inner1, "start", NULL);
    addString(inner1, "bar", "baz");
    addNumber(inner1, "dup_group", 0);
    users = createList(inner1, "users");
    user = createObject(users, "includetest-fooandbar");
    includeThis(user, "foo:bar");
    addString(user, "a", "cheese");
    addString(user, "b", "2001.020");
    addNumber(user, "c", 10);
    addString(user, "date", "01-jun-2029");
    destroy(user);
    destroy(users);
    user = createObject(users, "excludetest-onlyversion");
    includeThis(user, "expire:use:name");
    addString(user, "name", "milk");
    addString(user, "version", "2%");
    addNumber(user, "gallons-used", 10);
